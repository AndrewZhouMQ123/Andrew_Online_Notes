...
const Calculator = () => {
  ...
  const calculate = () => {
    try {
      ...
      const calculationResult = new Function(
        "context",
  ...
  const funcGridItems: GridItemProps[] = [
    ...
    {
      text: "X^y",
      action: { onClick: () => setInput("exp(" + input + ", )") },
    },
    ...
    {
      text: "floor",
      action: { onClick: () => setInput("floor(" + input + ")") },
    },
    {
      text: "ceil",
      action: { onClick: () => setInput("ceil(" + input + ")") },
    },
  ...
  const numGridItems: GridItemProps[] = [
    ...
    {
      text: "Ans",
      action: { onClick: () => setInput(result?.toString() || "") },
    },
  ];
      ...
      action: {
        onClick: () => {
          ...
          const numericValue = parseFloat(
            isPercentage ? input.slice(0, -1) : input
          );
          try {
