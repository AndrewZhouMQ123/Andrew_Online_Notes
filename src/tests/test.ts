...
mocha.setup('bdd');
mocha.checkLeaks(); // Check for memory leaks
...
describe ("drawArrow", function() {
  ...
  const drawArrowStub = function(ctx: CanvasRenderingContext2D, x1: number, y1: number, x2: number, y2: number, lineWidth: number, strokeStyle: string) {
    return expectedArrow;
  };

  it("draws arrow shape on the canvas", function() {
    // Replace diagramObj.drawArrow with the stub function
    assertDeepEqual(drawArrowStub(ctx, 30, 60, 30, 18, 3, 'black'), expectedArrow);
  });
});
...
describe ("drawboxModel", function() {
  ...
  it("draws box model on the canvas", function() {
    ...
    const drawboxModelStub = function() {
      return expectedRectangles;
    };

    it("draws box model on the canvas", function() {
      const rectangles = drawboxModelStub();
      assertDeepEqual(rectangles, expectedRectangles);
    });
  });
});
...
describe ("drawCSSsyntax", function() {
  ...
  const drawCSSsyntaxStub = function() {
    return {
  ...
  it("draws arrows on the canvas", function() {
    assertDeepEqual(arrows, expectedArrows);
  });

  it("draws texts on the canvas", function() {
    assertDeepEqual(texts, expectedTexts);
  });
