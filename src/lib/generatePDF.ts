  ...
  generatePdf<DataType>(data as DataType[], title, (items) => {
    const configs = [
      ...
      },
      {
        ...
        rowMapper: (item: InterfaceData) => [
          item.interface,
          item.description,
          item.methods_properties,
        ],
      },
    ];
    ...
    const configs = [
      {
        ...
        guard: (
          arr: ItemType[]
        ): arr is { attribute: string; desc: string }[] =>
